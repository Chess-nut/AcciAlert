...
const FILTER_CHIPS = [
  { label: "All",            value: "all"                 },
  { label: "🚗 Traffic",     value: "traffic accident"    },
  { label: "🌊 Flooding",    value: "flooding"            },
  { label: "🛣️ Pothole",     value: "pothole"             },
  { label: "🔥 Fire",        value: "fire"                },
  { label: "🚧 Road Debris", value: "road debris"         },
  { label: "🚦 Broken Sign", value: "broken traffic sign" },
];
...
export default function HomeScreen() {
  ...
  // ─── Derived: search + filter applied in memory ───────────────────────────
  ...
  const filteredReports = useMemo(() => {
    ...
    if (activeFilter !== "all") {
      result = result.filter((r) =>
        r.incidentType.toLowerCase().includes(activeFilter)
      );
    }

    // Text search across incidentType, description, address
    ...
    return result;
  }, [recentReports, activeFilter, searchQuery]);
  ...
  const startDummyCall = (name: string, number: string) => {
    ...
    setCallerName(name); setCallerNumber(number); setCallSeconds(0);
    setIsConnected(false); setIsCallActive(true);
    connectionTimeoutRef.current = setTimeout(() => { setIsConnected(true); connectionTimeoutRef.current = null; }, 1200);
  };

  const endDummyCall = () => {
    if (connectionTimeoutRef.current) { clearTimeout(connectionTimeoutRef.current); connectionTimeoutRef.current = null; }
    setIsCallActive(false); setIsConnected(false);
    setCallerName(""); setCallerNumber(""); setCallSeconds(0);
  };
  ...
  const getIncidentEmoji = (t: string) => {
  ...
  const formatTimeAgo = (timestamp?: Timestamp) => {
    ...
    const elapsed = Math.max(0, Math.floor((Date.now() - timestamp.toDate().getTime()) / 1000));
    ...
    <View style={styles.containerWrapper}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View style={styles.headerContent}>
            <View style={styles.logo}>
              <MaterialCommunityIcons name="shield-check" size={44} color={BRAND} />
            </View>
            <View>
        ...
        <View style={styles.statsContainer}>
          {[
            ...
            <View key={stat.label} style={styles.statCard}>
              <MaterialCommunityIcons name={stat.icon as any} size={22} color={BRAND} />
        ...
        <View style={styles.section}>
          <View style={styles.quickDialContainer}>
            <View style={styles.quickDialHeader}>
              <MaterialCommunityIcons name="phone" size={20} color={BRAND} />
        ...
        <View style={styles.section}>
          ...
          {/* Report list / empty states */}
          {reportsLoading ? (
            <Text style={styles.recentMetaText}>Loading recent reports...</Text>
          ) : filteredReports.length === 0 ? (
            <View style={styles.emptyState}>
              <Text style={styles.emptyEmoji}>🔍</Text>
              <Text style={styles.emptyTitle}>No reports found in this area</Text>
              <Text style={styles.emptySubtitle}>
                {searchQuery.length > 0
                  ? `No results for "${searchQuery}". Try a different keyword.`
                  : "No reports match the selected filter yet."}
              </Text>
...
const styles = StyleSheet.create({
  ...
  // Call Modal
  callModalContainer:  { flex: 1, backgroundColor: "#1f1f1f", justifyContent: "space-between", alignItems: "center", paddingTop: 80, paddingBottom: 48 },
  callModalTop:        { alignItems: "center", paddingHorizontal: 24 },
  callLabel:           { fontSize: 28, fontWeight: "800", color: "#fff", textAlign: "center" },
  callNumber:          { marginTop: 10, fontSize: 16, color: "rgba(255,255,255,0.7)" },
  connectionRow:       { flexDirection: "row", alignItems: "center", gap: 8, marginTop: 18 },
  connectionDot:       { width: 10, height: 10, borderRadius: 5, backgroundColor: "#757575" },
  connectionDotActive: { backgroundColor: "#43a047" },
  connectionText:      { fontSize: 14, color: "rgba(255,255,255,0.85)", fontWeight: "600" },
  callTimer:           { marginTop: 18, fontSize: 54, fontWeight: "700", color: "#fff", letterSpacing: 1 },
  callControlsRow:     { flexDirection: "row", justifyContent: "space-between", width: "84%", maxWidth: 360 },
  callControlButton:   { width: 88, height: 88, borderRadius: 44, backgroundColor: "rgba(255,255,255,0.12)", justifyContent: "center", alignItems: "center", borderWidth: 1, borderColor: "rgba(255,255,255,0.15)" },
  callControlText:     { marginTop: 6, fontSize: 12, color: "#fff", fontWeight: "600" },
  endCallSection:      { alignItems: "center" },
  endCallButton:       { width: 84, height: 84, borderRadius: 42, backgroundColor: "#d32f2f", justifyContent: "center", alignItems: "center", shadowColor: "#000", shadowOpacity: 0.25, shadowRadius: 8, elevation: 6 },
  endCallLabel:        { marginTop: 10, fontSize: 14, fontWeight: "700", color: "rgba(255,255,255,0.85)" },

  // Header
  header:        { backgroundColor: "#fff", paddingHorizontal: 20, paddingVertical: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderBottomColor: "#eee", borderBottomWidth: 1, elevation: 2, shadowColor: "#000", shadowOpacity: 0.06, shadowRadius: 4 },
  headerContent: { flexDirection: "row", alignItems: "center", gap: 12 },
  logo:          { backgroundColor: "#fff5f5", width: 56, height: 56, borderRadius: 28, justifyContent: "center", alignItems: "center", borderWidth: 2, borderColor: "#ffcdd2" },
  appName:       { fontSize: 22, fontWeight: "800", color: "#1a1a1a", letterSpacing: 0.5 },
  tagline:       { fontSize: 12, color: "#888", marginTop: 2 },
  sosButton:     { backgroundColor: BRAND, flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 10, borderRadius: 24, gap: 6, elevation: 3 },
  sosText:       { color: "#fff", fontWeight: "800", fontSize: 14, letterSpacing: 1 },

  // Stats
  statsContainer: { flexDirection: "row", padding: 16, gap: 12 },
  statCard:       { flex: 1, backgroundColor: "white", padding: 14, borderRadius: 12, alignItems: "center", shadowColor: "#000", shadowOpacity: 0.07, shadowRadius: 4, elevation: 2, gap: 4 },
  statNumber:     { fontSize: 22, fontWeight: "800", color: BRAND },
  ...
  // Emergency Dial
  quickDialContainer: { backgroundColor: "#fff1f1", borderRadius: 15, borderWidth: 1, borderColor: "#ef9a9a", padding: 14 },
  ...
  emergencyButton:    { width: "48%", minHeight: 120, backgroundColor: "#fff", borderRadius: 12, borderWidth: 1, borderColor: "#f0f0f0", paddingVertical: 14, paddingHorizontal: 12, alignItems: "center", justifyContent: "center", gap: 10, elevation: 2, shadowColor: "#000", shadowOpacity: 0.08, shadowRadius: 4 },
  emergencyEmoji:     { fontSize: 30, lineHeight: 34 },
  emergencyLabel:     { color: "#444", fontWeight: "600", fontSize: 12, textAlign: "center" },
  emergencyNumber:    { color: BRAND, fontSize: 14, fontWeight: "800", textAlign: "center" },

  // Section header
  sectionHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 12 },
  sectionTitle:  { fontSize: 18, fontWeight: "800", color: "#1f1f1f" },
  viewAllText:   { color: BRAND, fontWeight: "700", fontSize: 13 },
  ...
  // Filter chips
  chipsRow: { flexDirection: "row", gap: 8, paddingBottom: 14, paddingRight: 4 },
  chip:         { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 999, backgroundColor: "#ebebeb", borderWidth: 1, borderColor: "#ddd" },
  ...
  recentMetaText: { fontSize: 13, color: "#777", fontWeight: "500", textAlign: "center", paddingVertical: 14 },

  // Empty state
  emptyState:    { alignItems: "center", paddingVertical: 36, paddingHorizontal: 24 },
  emptyEmoji:    { fontSize: 40, marginBottom: 12 },
  emptyTitle:    { fontSize: 16, fontWeight: "800", color: "#333", marginBottom: 6 },
  emptySubtitle: { fontSize: 13, color: "#888", textAlign: "center", lineHeight: 20, marginBottom: 16 },
  clearBtn:      { backgroundColor: BRAND, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 999 },
  clearBtnText:  { color: "#fff", fontWeight: "700", fontSize: 13 },

  // Report card
  reportCard:        { backgroundColor: "#fff", borderWidth: 1, borderColor: "#ececec", borderRadius: 15, padding: 14 },
  reportCardHeader:  { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 10, gap: 10 },
  reportTitle:       { flex: 1, fontSize: 16, color: "#161616", fontWeight: "800" },
  statusBadge:       { flexDirection: "row", alignItems: "center", gap: 4, borderWidth: 1, borderColor: "#d0d0d0", borderRadius: 999, paddingHorizontal: 8, paddingVertical: 4, backgroundColor: "#fff" },
  statusBadgeText:   { color: "#666", fontSize: 11, fontWeight: "700" },
  severityBadge:     { alignSelf: "flex-start", borderRadius: 999, paddingHorizontal: 10, paddingVertical: 5, marginBottom: 10 },
  severityBadgeText: { color: "#fff", fontWeight: "700", fontSize: 12 },
  reportImageWrap:   { width: "100%", borderRadius: 12, overflow: "hidden", borderWidth: 1, borderColor: "#ececec", marginBottom: 10 },
  reportImage:       { width: "100%", height: 170, backgroundColor: "#f0f0f0" },
  ...
  // FAB
  fab: { position: "absolute", bottom: 24, right: 24, width: 60, height: 60, borderRadius: 30, backgroundColor: BRAND, justifyContent: "center", alignItems: "center", elevation: 8, shadowColor: "#000", shadowOpacity: 0.3, shadowRadius: 8 },
});